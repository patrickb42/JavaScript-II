...
}); // THESE DIRECTIONS WERE NOT CLEAR; it should say something more like "allCaps should be assigned an array consisting of just the capitalized names"
// console.log(allCaps);
...
const ticketPriceTotal = runners.reduce(function (acc, runner) {
...
// Problem 1
// make a new array with only first name, last name and company name
const trimmedArray = runners.map(function (runner) {
    let { first_name, last_name, company_name } = runner;
    return Object.freeze({ first_name, last_name, company_name });
});
console.log(trimmedArray);
// Problem 2
// count the number of players on team Wordware
const wordwareCount = trimmedArray.reduce(function (acc, runner) {
    return acc + (runner.company_name.localeCompare("Wordware") === 0 ? 1 : 0);
}, 0);
console.log(wordwareCount);
// Problem 3
// Someone stole all of the money; make everyone's donations 0
runners.forEach(function (runner) {
    runner.donation = 0;
});
console.log(runners);
